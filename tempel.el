...
(defcustom tempel-template-sources
  (list #'tempel-path-templates 'tempo-tempel--templates)
...
(defvar tempo-tempel--templates nil
  "Used in `tempel-tempo-define-template' for compatibility with tempo.")
...
If a field was added, return it."
  (setq prompt
        (cond
         ((and (stringp prompt) (if tempel-tempo-interactive t noinsert))
          (read-string prompt))
...
(defun tempel-tempo-define-template (name elements &optional tag documentation _taglist)
  ;; TODO: I dont think taglist will be supported
  (let* ((command-name (intern (concat "tempel-template-" name))))
    (fset command-name (lambda (&optional arg)
			 (:documentation
			  (or documentation (concat "Insert a " name ".")))
			 (interactive "*P")
                         (tempel--insert
                          elements (and tempel-tempo-insert-region arg (tempel--region)))))
    (when tag (cl-pushnew `(,(intern tag) ,@elements) tempo-tempel--templates))
    command-name))
...
;; TODO: This is to define templates interactively via functions done
;; in `tempel-tempo-define-template' (?)
...
or to `tempo-tempel--templates' if TAG-LIST is nil.  If TAG was already
in the list, replace its template with TEMPLATE."

  (interactive "sTag: \nCTemplate: "))

;; TODO: This should be a mix between `tempel-complete' and `tempel-expand'
;;;###autoload
...
(defalias 'tempel-tempo-expand-if-complete 'tempel-expand
